            ...
            <table class="table table-striped table-bordered dataTable" id="tab" >
                ...
                @foreach($data as $v)
                    <tr>
                        ...
                        <td>{{$v->DeviceName}}</td>
                        <td>
                            <a href="jacascript::void(0)" ><button type="button" class="btn btn-primary w-min-xs  waves-effect waves-light temp" data-toggle="modal" data-target="#terminal" data-action="{{$v->id}}">温度详细信息</button></a>
    ...
    <script>
        ...
        $(function () {
            $("body").delegate('.table-striped tr', //会为符合条件的现有标签和未来标签都绑定事件
                'click', function () {
                    $.post("{{ url('/admin/ajax/getTerminalTemp') }}/"+ $('.table-striped tr').eq($(this).index()+1).find('.temp').data('action'),
