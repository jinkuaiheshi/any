            ...
            <table class="table table-striped table-bordered dataTable" id="tab" >
                ...
                @foreach($data as $v)
                    <tr>
                        ...
                        <td>{{$v->provinceName.$v->cityName.$v->countyName.$v->street}}</td>
                        <td>{{$v->latitude}}</td>
                        <td>{{$v->longitude}}</td>

                        <td>{{$v->projectName}}</td>
                        <td>
                            <a href="{{url('admin/terminal/info').'/'.$v->projectCode}}" ><button type="button" class="btn btn-info w-min-xs  waves-effect waves-light terminal" >项目管理</button></a>
